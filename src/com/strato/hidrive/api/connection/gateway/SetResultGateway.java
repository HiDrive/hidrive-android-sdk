...
public abstract class SetResultGateway<T> extends HiDriveDomainGateway<List<T>> {
	...
	public SetResultGateway() {
		super();
	}

	protected List<T> prepareResult(Response<String> response) throws Exception {
		List<T> setResult=new ArrayList<T>();
		for (DataReader dataReader:prepareDataReaderList(response))
		{
			setResult.add(prepareObject(dataReader));
		}
		return setResult;
	}
